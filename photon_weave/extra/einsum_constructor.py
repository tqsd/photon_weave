...
def apply_operator_matrix(state_objs: list, operator_objs: list) -> str:
    ...
    Examples
    --------
    Apply an operator to a reshaped density matrix:

    .. code-block:: python

        state = original_state.reshape(dims)
        einsum = apply_operator_matrix(state_objs, operator_objs)
        new_state = oe.contract(
            einsum, operator, state, jnp.conj(operator), backend="jax"
        ).reshape(new_dims)

    where:
    - ``dims``/``new_dims`` are the dimensions before/after the operation.
    - ``state_objs`` lists the states in tensor order.
    - ``operator_objs`` lists the targets the operator acts on.
    - ``operator`` is the matrix to apply.
    - ``einsum`` is the string generated by :func:`apply_operator_matrix`.
    """
