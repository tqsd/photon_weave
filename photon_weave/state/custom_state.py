...
class CustomState(BaseState):
    ...
    def contract(self, final: ExpansionLevel = ExpansionLevel.Label, tol:float=1e-6) -> None:
        ...
        if self.expansion_level is ExpansionLevel.Matrix and final < ExpansionLevel.Matrix:
            ...
            assert self.state.shape == (self.dimensions, self.dimensions), "Dimensions do not match"
    ...
    def apply_kraus(self, operators: List[Union[np.ndarray, jnp.ndarray]], identity_check:bool=True) -> None:
        ...
        for op in operators:
            if op.shape != (self.dimensions, self.dimensions):
                raise ValueError(f"Kraus operator has incorrect dimensions: {op.shape}, expected ({self.dimensions},{self.dimensions})")
