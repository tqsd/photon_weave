...
Fig.1 depicts a complicated scenario where the Beam Splitters are lossy, and each step in the experiment is tracked. ![Figure 1. Simulation of lossy Mach-Zender Interferometers. The simulation tracks the state evolution throughout the experiment. The losses here are photon absorption.](circuit.png){label="fig.1"}.

In Fig.2, You can see the performance regarding speed where the steps represent the number of applied operations. ![Figure 2. Comparison between Photon Weave, Qiskit, and QuTip regarding simulation time and the required space to simulate the experiment in \ref{fig.1}](lossy_circuit_paper-2.png)
