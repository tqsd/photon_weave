...
def composite_plan_benchmark(use_jit: bool) -> float:
    ...
    def jitted_apply():
        ...
        for _ in range(reps):
            rho = apply_operation_matrix(
                states, targets, rho, bs_op, meta=plan_ops, use_contraction=True
            )
        return rho
    ...
    apply_jit_fn = (
        jax.jit(jitted_apply) if use_jit else jitted_apply
    )
    # warm and time apply
    apply_jit_fn()
    if use_jit:
...
def main() -> None:
    ...
    results = (
        ...
        BenchmarkResult(
            "composite measure", comp_meas_legacy, comp_meas_jit, kind="measure"
        ),
    )
