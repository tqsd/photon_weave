...
class TestPolarizationMeasurement(unittest.TestCase):
    ...
    def test_measure_H_state_vector(self) -> None:
        ...
        self.assertTrue(
            pol.measured,
            "Polarization must have measurement=True after measurement.",
        )
        for item in [
            pol.envelope,
    ...
    def test_measure_R_state_vector(self) -> None:
        ...
        self.assertEqual(
            m[pol],
            0,
            "Measurement outcome when measuring R must always be 0,"
        ...
        self.assertEqual(
            m[pol],
            1,
            "Measurement outcome when measuring R must always be 1,"
            "when seed is set to 3",
        )
        self.assertTrue(
            pol.measured,
            "Polarization must have measurement=True after measurement.",
    ...
    def test_measure_R_density_matrix(self) -> None:
        ...
        pol.expand()
        m = pol.measure()
        self.assertEqual(m[pol], 0)
        self.assertTrue(pol.measured)
        for item in [
            pol.envelope,
            pol.composite_envelope,
            pol.state,
