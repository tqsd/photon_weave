...
class TestSWAPOperator(unittest.TestCase):
    def test_swap_operator_vector(self) -> None:
        ...
        ce.apply_operation(op, env1.polarization, env2.polarization)
        self.assertTrue(
            jnp.allclose(env1.polarization.trace_out(), jnp.array([[1, 0], [0, 0]]))
        )

        self.assertTrue(
            jnp.allclose(env2.polarization.trace_out(), jnp.array([[0, 0], [0, 1]]))
        )
...
class TestCSWAPOperator(unittest.TestCase):
    def test_CSWAP_vector(self) -> None:
        ...
        self.assertTrue(
            jnp.allclose(env1.polarization.trace_out(), jnp.array([[0, 0], [0, 1]]))
        )
        self.assertTrue(
            jnp.allclose(env2.polarization.trace_out(), jnp.array([[1, 0], [0, 0]]))
        )
        self.assertTrue(
            jnp.allclose(env1.polarization.trace_out(), jnp.array([[0, 0], [0, 1]]))
        )
